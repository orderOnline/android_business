...
public class CardBackFragment extends Fragment{
	...
	OnBackCardOrderSelectedListener mListener;
	Activity activity;
    ...
    public interface OnBackCardOrderSelectedListener {
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            mListener = (OnBackCardOrderSelectedListener) activity;
        } catch (ClassCastException e) {
            throw new ClassCastException(activity.toString() + " must implement OnBackCardOrderSelectedListener");
        }
    }
    
    public CardBackFragment( String orderData ) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
    	...
    	View mainview = inflater.inflate(R.layout.fragment_card_back, container, false);
		...
		btn_accept.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				mListener.onBackCardOrderSelected(Constants.JSON_ORDER_STATUS_ACCEPTED, view);
				/*AppEventsController.getInstance().getModelFacade().getResModel().getPendingOrderItems().remove(order_id);
				currentItem.setOrder_status(Constants.JSON_ORDER_STATUS_ACCEPTED);
				AppEventsController.getInstance().getModelFacade().getResModel().addOrders(currentItem);
				if( newItems.size() > 0 ){
		...
		btn_decline.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				AppEventsController.getInstance().getModelFacade().getResModel().addOrders(currentItem);
				mListener.onBackCardOrderSelected(Constants.JSON_ORDER_STATUS_DECLINED, view);
				/*AppEventsController.getInstance().getModelFacade().getResModel().getPendingOrderItems().remove(order_id);
				currentItem.setOrder_status(Constants.JSON_ORDER_STATUS_DECLINED);
				AppEventsController.getInstance().getModelFacade().getResModel().addOrders(currentItem);
				if( newItems.size() > 0 ){
